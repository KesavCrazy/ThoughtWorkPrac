...
public void NoSeatMessage()
{
	try {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        WebElement noSeatMessage = wait.until(ExpectedConditions.visibilityOf(noSeatMsg));
        Assert.assertTrue(noSeatMessage.getText().contains("Sorry, there are no more seats available."));
        System.out.println("Sorry, there are no more seats available."+"Is displayyed");
